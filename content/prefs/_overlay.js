if (!window.extLoad)
    var extLoad = {
        ...
        init: function () {
            ...
            for (var loader in extLoad.loaders)
                if (extLoad.loaders[loader][1])
                    extLoad.loaders[loader][1]();
...
var aiosKeyconfig = {
    ...
    loadkeys: function (name) {
        ...
        for (i = 0; i < this.keys.length; i++) {
            ...
            if (key[0])
                node.setAttribute("modifiers", key[0]);
            if (key[1])
                node.setAttribute("key", key[1]);
            if (key[2])
                node.setAttribute("keycode", key[2]);
        }
    }
};
