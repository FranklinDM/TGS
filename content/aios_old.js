...
function aios_initSidebar() {
    ...
    if (!aios_getBoolean(fx_sidebarBox, 'aiosInit')) {
        ...
        if (aios_toolbar)
            aios_toolbar.setAttribute('iconsize', document.getElementById('nav-bar').getAttribute('iconsize'));
    ...
    try {
        ...
        if (sidebarInit == "open")
            toggleSidebar(fx_sidebarBox.getAttribute('aiosLastPanel'), true);
        ...
        if (toolbarInit != 2)
            aios_toolbar.setAttribute('hidden', !toolbarInit);
        if (switchInit != 2)
            aios_toggleBox.setAttribute('hidden', !switchInit);
    } catch (e) {}
    ...
    try {
        ...
        if (!vButtons)
            fx_mainWindow.setAttribute('aiosVButtons', 'false');
...
function aios_setSidebarOrient() {
    ...
    try {
        ...
        if (sidebarOrient == 2)
            fx_mainWindow.setAttribute('aiosOrient', 'right');
...
function aios_observeSidebar(mode) {
    ...
    if (showSwitch && !aios_isSidebarHidden())
        aios_toggleBox.setAttribute('hidden', false);
...
function aios_toggleSidebar(aMode, aForceOpen) {
    ...
    var prefstring = "key";
    if (aMode == elem_switch || aMode == "switch")
        prefstring = "switch";
    if (aMode == elem_tbb || aMode == "tbb")
        prefstring = "tbb";
    if (aMode == elem_close || aMode == elem_close2 || aMode == "close")
        prefstring = "close";
    ...
    try {
        ...
        if (mode == 4) {
            ...
            if (aios_collapseSidebar && forcePanel && fx_sidebarBox.getAttribute('aiosLastPanel') != forcePanel && !aios_isSidebarHidden())
                var closeNow = true;
...
function lwthemeColorHandler() {
    ...
    case false:
        ...
        if (ccl != "")
            fx_browser.style.background = ccl;
        break;
    default:
